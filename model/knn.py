...
def knn(train, test, airport_to_location):
    ...
    ##knn code
    knnregresor = KNeighborsRegressor(n_neighbors=5)
    knnregresor.fit(X, Y)

    y_pred = print(knnregresor.predict(X_test))

    mse = mean_squared_error(Y_test, knnregresor.predict(X_test))
    mae = mean_absolute_error(Y_test, knnregresor.predict(X_test))
    r2 = r2_score(Y_test, knnregresor.predict(X_test))

    print("mean squared error: ", mse)
    print("mean absolute error: ", mae)
    print("r2: ", r2)

    plt.scatter(knnregresor.predict(X_test), Y_test, label="predicted vs actual")
