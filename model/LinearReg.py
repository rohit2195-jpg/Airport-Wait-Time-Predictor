...
def CustomLinearRegression(train, test, airport_to_location):
    ...
    print("mean error: ", mean_absolute_error(Y_test, predictions))
    print("r2 score: ", r2_score( Y_test, predictions))
    print("\n\n prediction: ", predictions)
    print("\n\n test data: ", Y_test)

    fig, axes  = plt.subplots(2, 3, figsize=(12, 8))
    axes[0, 0].scatter(X["FlightCount"], Y)
    axes[0, 0].set_title('FlightCount to AvgWait')

    axes[0, 1].scatter(X["HourRange"], Y)
    axes[0, 1].set_title('TimeOfDay and Wait')

    axes[0, 2].scatter(X["temp"], Y)
    axes[0, 2].set_title('Temperature to Wait')

    axes[1, 0].scatter(X["wspd"], Y)
    axes[1, 0].set_title('Windspeed to Wait')

    axes[1, 1].scatter(X["Month"], Y)
    axes[1, 1].set_title('Month and Wait')

    axes[1, 2].scatter(X["BoothsUsed"], Y)
    axes[1, 2].set_title('Booths and Wait')
    plt.tight_layout()
    plt.show()
